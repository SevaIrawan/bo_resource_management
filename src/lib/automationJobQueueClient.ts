...
/** Enqueue lalu coba RUN; slot penuh → job tetap queued (bukan error). */
export async function enqueueAndTryRunAutomationJob(
  input: AutomationJobEnqueueInput,
): Promise<{ ok: true; jobId: string } | { ok: false; error: string }> {
  const enqueued = await enqueueAutomationJob(input);
  if (!enqueued.ok) return enqueued;
  await runAutomationJob(enqueued.jobId);
  return enqueued;
}

/** Coba RUN job yang sudah di-queue — gagal run (slot penuh) tidak throw. */
export async function tryRunEnqueuedAutomationJob(jobId: string): Promise<void> {
