...
**Versi aplikasi:** `1.0.37` (lihat `package.json`)  
...
## 4. Konfigurasi & installer (kondisi 1.0.37)
...
*Dokumen ini mencerminkan kondisi codebase per build **1.0.37**. Telegram Sync: kill sidecar by port saat quit + anti-orphan + Errno 22 soft (`SESSION_WARM_PENDING`) + sidecar v14; Job Queue header sticky + status Partial vs Failed; (1.0.36+) Errno 22 export serialize-first + setup anti-flicker; Join CSV hybrid; TG scrape left/migrated + `TG_SESSION_DEAD`; leave/delete outcomes; Job Queue basic Chat + Channel; AuthKeyDuplicated → re-login; Sync↔Job saling blokir; scrape WA hanya grup live; RPC `rm_commit_account_scrape` atomik (+ `is_owner`, migrasi **039**); PK master `(brand, platform, group_id)` — migrasi **036**; Job Queue batch split 30; Automatic account scrape factory reset.*
