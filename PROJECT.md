...
**Versi dokumen:** 2026-07-27  
**Versi aplikasi:** `1.0.33` (lihat `package.json`)  
...
## 4. Konfigurasi & installer (kondisi 1.0.33)
...
*Dokumen ini mencerminkan kondisi codebase per build **1.0.33**. Sync↔Job saling blokir dua arah; scrape WA hanya grup yang masih di akun; tulis DB lewat RPC `rm_commit_account_scrape` atomik; PK master `(brand, platform, group_id)` — migrasi **036** di Supabase; Job Queue batch split 30 grup per job + status partial = Failed; Automatic account scrape kontrak UI factory reset.*
